//全局变量，记录四个需要输出的切削力
var force_1,force_2,force_3,force_4;
var yuanshi1,yuanshi2,yuanshi3,yuanshi4;
...
function gra_dra(){
  ...
  document.getElementById('saveexpiamgebtn').class="btn btn-success";
...
//画图
	...
	for (var i=0;i<4;i++)
	{
		...
		cxt.moveTo(10+(Math.log(f[i])/Math.log(10)*250),510-(Math.log(fc1[i]/100)/Math.log(100))*500);
		cxt.fillText("("+(Math.round(f[i]*10))/10+","+(Math.round(fc1[i]*10))/10+")",10+(Math.log(f[i])/Math.log(10)*250),520+30*m-(Math.log(fc1[i]/100)/Math.log(100))*500);
		cxt.arc(10+(Math.log(f[i])/Math.log(10)*250),510-(Math.log(fc1[i]/100)/Math.log(100))*500,1.5,0,Math.PI*2,true);
		cxt.closePath();
		cxt.fill();
		//m=-m;
	}
...
lalala=lalala+"<tr><td><p><b>切削速度:</b><p></td><td><p>"+document.getElementById('qxsd').value+"m/min</p></td><td></td></tr>";
lalala=lalala+"<tr><td><p><b>进给量:</b><p></td><td><p>"+document.getElementById('jjl').value+"mm/r</p></td><td></td></tr></table>";
lalala=lalala+"<table class='table '><tr><td><p><b>背吃刀量(mm):</b></p></td><td><b>(1)</b></p></td><td><b>(2)</b></p></td><td><b>(3)</b></p></td><td><b>(4)</b></p></td></tr><tr><td></td>"
for (i=0;i<4;i++)
{
	lalala=lalala+"<td><p>"+f[i]+"</p></td>";
}
lalala=lalala+"</tr>";
lalala=lalala+"<tr><td><p><b>主切削力(N):</b></p></td><td><b>(1)</b></p></td><td><b>(2)</b></p></td><td><b>(3)</b></p></td><td><b>(4)</b></p></td></tr><tr><td></td>"
for (i=0;i<4;i++)
{
...
function donghua100(x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,s) {
	...
	for(var i=0;i<10;i++)
	{
		zhishu1[i]=Math.pow(f[i]*(1+((Math.random()*(-800))/10000)),x13)/1.18;
	}
...
function donghuabyphp(w) {
  ...
  //         var byphp100 =  xmlHttp.responseText;
	...
	//aaaa获取Fc值
	var cfc=document.getElementById('cfc').value;
	var xfc=document.getElementById('bcdl_11').value;
	var yfc=document.getElementById('bcdl_12').value;
	var kkfc=document.getElementById('kkfc').value;
	var klfc=document.getElementById('klfc').value;
