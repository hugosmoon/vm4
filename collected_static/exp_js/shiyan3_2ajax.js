...
function gra_dra(x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12) {
  if (!(expstatus)) {
    alert("您没有进行任何实验操作~")
    return false;
...
imagestatus=1;
	...
	var b=Array();
		...
		b.sort(function(a,b){return a>b?1:-1});//背吃刀量从小到大排序
	//温度计算
	var t=Array();
	for(i=0;i<10;i++)
	{
		var suijishu1=0.95+Math.random()/10;
		t[i]=suijishu1*223*Math.pow(q,zhishu1)*Math.pow(j,0.14)*Math.pow(b[i],0.04)*kr0*kkr;
		t[i]=(10*Math.round(t[i]))/10;
	}
...
//画图
	...
	for (i=6;i<10;i++)
	{
		cxt.moveTo(10+(Math.log(b[i])/Math.log(10)*250),510-(Math.log(t[i]/100)/Math.log(100))*500);
		cxt.lineTo(10+(Math.log(b[i+1])/Math.log(10)*250),510-(Math.log(t[i+1]/100)/Math.log(100))*500);
		cxt.fillText("("+b[i]+","+t[i]+")",10+(Math.log(b[i])/Math.log(10)*250),520+30*m-(Math.log(t[i]/100)/Math.log(100))*500);
		cxt.arc(10+(Math.log(b[i])/Math.log(10)*250),510-(Math.log(t[i]/100)/Math.log(100))*500,1.5,0,Math.PI*2,true);
		cxt.closePath();
		cxt.fill();
		//m=-m;
	}
...
else
{
 ...
 if(kr0==1.15)
{
	lalala=lalala+"前角:-15&deg<br>"
}
else if(kr0==1.1)
{
	lalala=lalala+"前角:-10&deg<br>"
}
else if(kr0==1.05)
{
	lalala=lalala+"前角:0&deg<br>"
}
else if(kr0==1)
{
	lalala=lalala+"前角:10&deg<br>"
...
{
	lalala=lalala+"前角:20&deg<br>"
}

lalala=lalala+"<b>切削速度</b>:"+document.getElementById('qxsd').value+"m/min<br>";
lalala=lalala+"<b>进给量</b>:"+document.getElementById('jjl').value+"mm/r<br>";
lalala=lalala+"<b>背吃刀量</b>(mm):<br>"
for (i=6;i<10;i++)
{
	lalala=lalala+b[i]+"&nbsp;&nbsp;&nbsp;&nbsp;";
}
lalala=lalala+"<br>";
lalala=lalala+"<b>切削温度/°C:<br>"
for (i=6;i<10;i++)
{
	lalala=lalala+t[i]+"&nbsp;&nbsp;";
}
...
document.getElementById('qxlcanshukuang2_2').innerHTML = lalala+'<input name="data_3_1" type="hidden" value="'+lalala+'">';
}
...
function donghua100(x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x15,s) {
  ...
  expstatus=1;
	...
	{
		var suijishu1=0.95+Math.random()/10;
		dhwd[i]=suijishu1*223*Math.pow(q,zhishu1)*Math.pow(j,0.14)*Math.pow(bb[i],0.04)*kr0*kkr;
		dhwd[i]=(10*Math.round(dhwd[i]))/10;
	}


		play(anniuid,bb[x15],j,q);
