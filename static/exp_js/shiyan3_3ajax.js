...
imagestatus=1;
	...
	var j=Array();
		...
		j.sort(function(a,b){return a>b?1:-1});//背吃刀量从小到大排序
	//温度计算
	var zhishu1;
	var t=Array();
	for(i=0;i<10;i++)
	{
		var zhishu1=0.26+(1/(50*j[i]+6.67));
		var suijishu1=0.95+Math.random()/10;
		t[i]=suijishu1*223*Math.pow(q,zhishu1)*Math.pow(j[i],0.14)*Math.pow(b,0.04)*kr0*kkr;
		t[i]=Math.round(t[i]);
	}
...
//画图
	...
	for (i=5;i<10;i++)
	{
		cxt.moveTo(10+(Math.log(10*j[i])/Math.log(10)*250),510-(Math.log(t[i]/100)/Math.log(100))*500);
		cxt.lineTo(10+(Math.log(10*j[i+1])/Math.log(10)*250),510-(Math.log(t[i+1]/100)/Math.log(100))*500);
		cxt.fillText("("+j[i]+","+t[i]+")",10+(Math.log(10*j[i])/Math.log(10)*250),520+30*m-(Math.log(t[i]/100)/Math.log(100))*500);
		cxt.arc(10+(Math.log(10*j[i])/Math.log(10)*250),510-(Math.log(t[i]/100)/Math.log(100))*500,1.5,0,Math.PI*2,true);
		cxt.closePath();
		cxt.fill();
		//m=-m;
	}
...
lalala=lalala+"<b>背吃刀量</b>:"+document.getElementById('bcdlx').value+"mm/r<br>";
lalala=lalala+"<b>进给量(mm):<br>"
for (i=5;i<10;i++)
{
	lalala=lalala+j[i]+"&nbsp;&nbsp;&nbsp;&nbsp;";
}
lalala=lalala+"<br>";
lalala=lalala+"<b>切削温度</b>(&deg;C):<br>"
for (i=5;i<10;i++)
{
	lalala=lalala+t[i]+"&nbsp;&nbsp;";
}
...
function donghua100(x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x15,s) {
	...
	for(i=0;i<10;i++)
	{
		var zhishu1=0.26+(1/(50*jj[i]+6.67));
		suijishu1=0.95+Math.random()/10;
		dhwd[i]=suijishu1*223*Math.pow(q,zhishu1)*Math.pow(jj[i],0.14)*Math.pow(b,0.04)*kr0*kkr;
		dhwd[i]=(10*Math.round(dhwd[i]))/10;
	}

		play(anniuid,b,jj[x15],q);
